...
class Solution {
	...
	public int[][] solution(int n, int[][] build_frame) {
		...
		for (int[] element : build_frame) {
			...
			if (sort == 0) {
				...
				if (action == 1) {
					// 설치 시
					if (y == 0) {
						// y좌표가 바닥이라면 문제가 없으므로 설치한다.
						pillarMap.put(pos, sort);
					} else {
						// 바닥이 아니라면 다른 기둥 위에 있거나, 보의 한쪽 끝 부분 위에 있는지 검사한다.
						if (isValidPillar(x, y, pillarMap, beamMap)) {
							pillarMap.put(pos, 0);
						}
						// 둘 다 만족하지 않을 경우엔 아무 행동을 취하지 않고 넘어간다.
					}
				} else {
					...
					if (pillarMap.containsKey(pos)) {
						...
						if (pillarMap.containsKey("" + x + "," + (y + 1)) ? isValidPillar(x, y + 1, pillarMap, beamMap)
								: true && beamMap.containsKey("" + x + "," + (y + 1))
										? isValidBeam(x, y + 1, pillarMap, beamMap)
										: true && beamMap.containsKey("" + (x - 1) + "," + (y + 1))
												? isValidBeam(x - 1, y + 1, pillarMap, beamMap)
												: true) {
				...
				if (action == 1) {
					...
					if (beamMap.containsKey(pos)) {
						...
						if (pillarMap.containsKey("" + x + "," + y) ? isValidPillar(x, y, pillarMap, beamMap)
								: true && pillarMap.containsKey("" + (x + 1) + "," + y)
										? isValidPillar(x + 1, y, pillarMap, beamMap)
										: true && beamMap.containsKey("" + (x - 1) + "," + y)
												? isValidBeam(x, y + 1, pillarMap, beamMap)
												: true && beamMap.containsKey("" + (x + 1) + "," + y)
														? isValidBeam(x - 1, y + 1, pillarMap, beamMap)
														: true) {
		...
		System.out.println("pillarMap = " + pillarMap.toString());
		System.out.println("beamMap = " + beamMap.toString());
	...
	public boolean isValidBeam(int x, int y, Map<String, Integer> pillarMap, Map<String, Integer> beamMap) {
		if (pillarMap.containsKey("" + x + "," + (y - 1)) && pillarMap.get("" + x + "," + (y - 1)) == 0
				|| pillarMap.containsKey("" + (x + 1) + "," + (y - 1))
						&& pillarMap.get("" + (x + 1) + "," + (y - 1)) == 0) {
			return true;
